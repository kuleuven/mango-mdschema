...
- You have a iRODS object (data object or collection) and you extract the metadata
...
When calling the `apply()` method on the `Schema` object, it will its
...
First we import what we need. The `Schema` class is the most important tool
in this package: it reads a schema from file, validates it and lets you validate,
...
metadata to the provide iRODS collection of data object, it will
check if there is already metadata linked to the schema, replaces it and
updates the metadata related to the schema version. This way you make
sure that the metadata is applied consist with what the ManGO portal does.

```python
my_schema.apply(my_object, my_metadata)
```

For all tree method (`apply`, `to_avus` and `validate`) you can pass the
parameters `set_defaults=False` and/or `convert=False` to disable the the
first 2 steps of the validation: assign defaults and conversion.
...
all values converted to their type defined by the schema fields.
...
As demonstrated below, passing the AVUs generated by `to_avus` back into
...
attributes, as well as the `prefix` used for all AVU names. The prefix is the
combination of a prefix given in the constructor (by default ‘mgs’) and the name
of the schema. For example, initializing with `Schema('book-v2.0.0-published.json', 'irods')`
would generate the prefix ‘irods.book’ for all the metadata related to this schema.
...
2. All values in the metadata dictionary (with added defaults) are convert
...
> **Warning**: Setting defaults in the ManGO portal works different then this
...
For all tree Schema methods (`apply`, `to_avus` and `validate`) you can pass the
parameters `set_defaults=False` and/or `convert=False` to disable the
first 2 steps of the validation: assign defaults and conversion.
...
on the schema and add the generated list to `item.metadata.apply_atomic_operations()`,
