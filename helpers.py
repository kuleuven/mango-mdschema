...
def check_metadata(schema, metadata: dict, verbose: bool = False, index: int = None) -> list:
    ...
    missing_required = [field_name for field_name, default in schema.required_fields.items()
    ...
    if verbose:
        ...
        if len(go_to_default) > 0:
            default_pairs = ', '.join(f"{field_name} ({schema.required_fields[field_name]})" for field_name in go_to_default)
        ...
        missing_non_required = [field_name for field_name, field in schema.fields.items()
    ...
    return [meta for meta_list in to_use.values() for meta in meta_list]