...
def check_metadata(
    ...
    Args:
        ...
        index (str, optional): Index of the composite field, for the units of its subfields. Defaults to None.
    ...
    if len(missing_required) > 0:
        raise KeyError(
            f"The following required fields are missing and there is no default: {', '.join(missing_required)}."
        )
    ...
    if verbose:
        ...
        if len(go_to_default) > 0:
            ...
            logging.warning(
                f"The following required fields are missing and the default value will be used: {default_pairs}"
            )
        ...
        if len(missing_non_required) > 0:
            logging.warning(
                f"The following non required fields are missing: {', '.join(missing_non_required)}."
            )
        ...
        if len(to_use) < len(metadata):
            ...
            logging.warning(
                f"The following fields do not belong to the schema and will be ignored: {extras}."
            )
